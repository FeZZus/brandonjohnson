  ...
  return new Promise((resolve, reject) => {
    ...
    process.stdin.once("data", (chunk) => {
      ...
      if (isNaN(n) || n < 1 || n > entries.length) {
        reject(new Error(`Invalid selection "${String(chunk).trim()}" — enter a number between 1 and ${entries.length}`));
        return;
      }
...
async function main() {
  ...
  console.log(`\nFetching applications… [${lng}, ${lat}] r=${radius}m | ${dateFrom} → ${dateTo}`);

  const { applications, pages } = await fetchAllApplications({ lng, lat, radius, dateFrom, dateTo });

  console.log(`\n=== Planning Analytics: ${name} [${lng}, ${lat}] r=${radius}m | ${dateFrom}–${dateTo} ===`);
  console.log(`Total fetched: ${applications.length} (${pages} page${pages !== 1 ? "s" : ""})`);
